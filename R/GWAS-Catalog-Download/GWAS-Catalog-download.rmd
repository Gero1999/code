...
```{r}
phenotype_regex = '.*[Cc]ancer$'
min_sample_size = 100
populations = c("European", "British", "Finish", "German")
nr_studies_per_phenotype = 2
...
```{r}
# Delete previous older files of the gwas catalog if they exist
...
gsub('[a-zA-Z,.]','',gwcat$`INITIAL SAMPLE SIZE`) %>% 
...
gwcat = gwcat %>% 
  ...
  select(sample_size, ancestry, `DISEASE/TRAIT`, PUBMEDID, DATE, LINK, STUDY, 
...
for (GCST in gwcat$`STUDY ACCESSION`[1:3]){
